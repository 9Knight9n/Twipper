...
def extract_tf_weight(words_frequency_dict):
    sum_value = sum(words_frequency_dict.values())

    top_items = heapq.nlargest(30, words_frequency_dict.items(), key=itemgetter(1))
...
def apply(tweet:str):
    ...
    results = extract_tf_weight(words_frequency_dict)
